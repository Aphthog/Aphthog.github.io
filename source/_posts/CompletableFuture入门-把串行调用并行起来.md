...
213ms 接近三个里最慢的那个（200ms）再加上调度开销，而不是三者之和。

注意这里有个不显眼的细节：**并行版用的是 5 个线程的池子，而不是 3 个。** 如果你给的是固定 3 线程的池，还要考虑三个任务会不会真的同时在跑——池子太小，`supplyAsync` 提交的任务一样会排队，那就白并行了。指定池子的时候，容量要按"能同时进行的任务数"来估。
